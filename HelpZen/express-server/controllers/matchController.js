...
exports.addNewMatchToQueue = async (matchData) => {
  if (
    // Check if required fields exist in matchData
    matchData.userId &&
    matchData.userStatus &&
    matchData.technology &&
    matchData.language
  ) {
    const prevMatchWithUserId = await Match.findOne({
      userId: matchData.userId,
    }).exec();
    let result = null;
    // Check if the user has already created a match before
    if (prevMatchWithUserId) {
      result = await prevMatchWithUserId.updateOne(matchData).then(() => {
        // Update the existing match object for the user
...
exports.findMatchForId = async (matchId) => {
  if (matchId) {
    ...
    if (matchObject1) {
      ...
      const userStatusToFind =
        matchObject1.userStatus === "helpee" ? "helper" : "helpee";
      const matchObject2 = await Match.findOne({
        userStatus: userStatusToFind,
        technology: matchObject1.technology,
        language: matchObject1.language,
      })
        .exec()
        .catch(() => false);
